...
test('instruction_specific_security_test_post_the_internet_herokuapp_com_login', async ({ page, request }) => {
  // Set timeout for complex security tests
  test.setTimeout(60000);
  
  console.log('🔒 Instruction-Specific Security Test');
  console.log('📋 Original Instruction: Open login page.Capture session cookie value.Login using valid credentials.Capture session cookie value again.');
  console.log('🎯 URL: https://the-internet.herokuapp.com/login');
  ...
  const testExecution: TestExecution = {
    steps: [],
    startTime: Date.now(),
    instruction: 'Open login page.Capture session cookie value.Login using valid credentials.Capture session cookie value again.'
  };
  ...
  console.log('\n🚀 Executing 5 instruction-specific steps:');\n\n  // Step 1: OPEN_PAGE
  
  // OPEN_PAGE: Open the page as instructed
  stepCounter++;
  console.log(`${stepCounter}. OPEN_PAGE - LOGIN_PAGE`);
  console.log('   Opening page: https://the-internet.herokuapp.com/login');
  ...
  try {
    ...
    // Use default credentials if none specified in instruction
    if (!instructionData.username && !instructionData.email) {
      credentials4.username = 'tomsmith';
      credentials4.password = 'SuperSecretPassword!';
    }
    
    console.log(`   Using credentials: ${credentials4.username} / [password hidden]`);
    ...
    for (const selector of usernameSelectors) {
      try {
        await page.waitForSelector(selector, { timeout: 2000 });
        await page.fill(selector, credentials4.username);
        console.log(`   ✅ Username filled using selector: ${selector}`);
        usernameFieldFound = true;
        break;
      } catch (e) {
    ...
    if (!usernameFieldFound) {
      console.log('   ⚠️  Username field not found, trying generic approach');
      try {
        await page.fill('input[type="text"]:first-of-type', credentials4.username);
      } catch (e) {
        console.log('   ⚠️  Could not fill username field');
      }
    }
    ...
    for (const selector of passwordSelectors) {
      try {
        await page.waitForSelector(selector, { timeout: 2000 });
        await page.fill(selector, credentials4.password);
        console.log(`   ✅ Password filled using selector: ${selector}`);
        passwordFieldFound = true;
        break;
      } catch (e) {
    ...
    if (!passwordFieldFound) {
      console.log('   ⚠️  Password field not found, trying generic approach');
      try {
        await page.fill('input[type="password"]:first-of-type', credentials4.password);
      } catch (e) {
        console.log('   ⚠️  Could not fill password field');
      }
    }
  ...
  try {
    // Get all cookies from current page context
    const allCookies5 = await page.context().cookies();
    
    // Store cookies with unique variable name
    sessionCookies['step5'] = allCookies5;
    
    if (allCookies5.length > 0) {
      // Look for session cookies (common patterns)
      const sessionCookie5 = allCookies5.find(cookie => 
        /^(JSESSIONID|PHPSESSID|session|sessionid|connect\.sid|sid|_session)$/i.test(cookie.name)
      );
      
      if (sessionCookie5) {
        console.log(`   Session cookie found: ${sessionCookie5.name}=${sessionCookie5.value.substring(0, 20)}...`);
        sessionCookies['session5'] = sessionCookie5.value;
      } else {
        // Store first cookie as fallback
        const firstCookie = allCookies5[0];
        console.log(`   First cookie captured: ${firstCookie.name}=${firstCookie.value.substring(0, 20)}...`);
        sessionCookies['first5'] = firstCookie.value;
      }
      
      console.log(`   Total cookies captured: ${allCookies5.length}`);
    } else {
      console.log('   No cookies found in current context');
      sessionCookies['step5'] = [];
    }
  ...
  try {
    expect(testExecution.steps.length, 'All instruction steps should be executed').toBe(5);
